...
void SHA1Transform(uint32_t state[5], const uchar buffer[64]);
void SHA1Init(SHA1_CTX * context);
void SHA1Update(SHA1_CTX * context, const uchar *data, uint32_t len);
...
void 
SHA1Update(SHA1_CTX * context, const uchar *data, uint32_t len) {
    uint32_t i;
    uint32_t j;
...
void 
SHA1Final(uchar digest[20], SHA1_CTX * context) {
    unsigned i;
